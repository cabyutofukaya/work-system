...
class SalesController extends Controller
{
    ...
    public function type_index($type): Response|ResponseFactory
    {
        ...
        $select_year = [];
        for ($limit_year = date('Y', strtotime('-5 year')); $limit_year < date('Y', strtotime('+1 year')); $limit_year++) {
            array_push($select_year, (int) $limit_year);
        }

        $year_list = [];
        for ($limit_year = date('Y', strtotime('-5 year')); $limit_year <= date('Y', strtotime('+4 year')); $limit_year++) {
    ...
    public function category_index($category, $year): Response|ResponseFactory
    {
        ...
        $year_list = [];
        for ($limit_year = date('Y', strtotime('-5 year')); $limit_year <= date('Y', strtotime('+2 year')); $limit_year++) {
            array_push($year_list, (int) $limit_year);
        }
