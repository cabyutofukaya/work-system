<template>
  <Layout>
    <div>
        ...
        <v-card-text>
          <v-row>
            ...
            <v-col cols="12" sm="" :class="{'text-right': $vuetify.breakpoint.smAndUp}">
              ...
              <Button
                  v-if="Number(report['user_id']) === Number($page.props.auth.user.id)"
                  :small="$vuetify.breakpoint.xs"
                  class="ml-2"
                  :to="$route('reports.edit',{report : report['id']})"
              >
                <v-icon left>
                  mdi-pencil
                </v-icon>
            ...
            <v-card-text>
              <div class="report-description-list">
                <v-row class="grey lighten-3 px-6">
                  <v-col><h3>{{ report_content["type_name"] }}</h3></v-col>
                </v-row>
                  ...
                  <v-col cols="12" sm="">
                    ...
                    <div class="text-right mt-2">
                      <v-chip
                          small class="mr-2"
                          color="primary"
                          :outlined="!report_content['has_own_like']"
                          style="cursor:pointer"
                          @click.native="reportContentLikeChange(report_content['id'])"
                      >
        ...
        <v-card-text>
          <v-row>
            ...
            <v-col cols="12" sm="" class="text-right">
              <Link as="Button"
                    :small="$vuetify.breakpoint.xs"
                    @click.native="formReportCommentDialog = true"
              >
                <v-icon left>
                  mdi-plus
                </v-icon>
      ...
      <!-- コメント入力ダイアログ -->
      <v-dialog
          v-model="formReportCommentDialog"
      ...
      >
        <v-card flat tile>
            ...
            <v-list>
              <v-list-item>
                <v-textarea
                    ...
                    v-model="formReportComment.comment"
                    :error="Boolean(formReportComment.errors.comment)"
                    :error-messages="formReportComment.errors.comment"
                ></v-textarea>
              </v-list-item>
            </v-list>
          </v-card-text>
          ...
          <v-card-text class="text-center">
            <Link as="Button"
                  color="primary"
                  @click.native="reportCommentCreate"
          ...
          <v-card-text class="text-right">
            <Link as="Button"
                  class="mt-4"
                  :small="$vuetify.breakpoint.xs"
                  @click.native="resetReportCommentDialog"
            >
              <v-icon>
                mdi-close
              </v-icon>
