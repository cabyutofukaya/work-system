        ...
        <v-card-text class="pt-0" v-if="report['report_comments'].length">
          <div v-for="report_comment in report['report_comments']" :key="report_comment.id">
            ...
            <v-row :class="{ 'grey': report_comment.is_readed == 1, 'lighten-4': report_comment.is_readed == 1 }">
              ...
              <v-col cols="12" sm="7">
                <span style="white-space: pre-line;font-size: smaller;" v-if="report_comment.mention_name">@{{
        ...
        <v-card flat tile>
          ...
          <v-card-text>
            <v-list>
              ...
              <v-list-item>
                <v-select dense hint="メンションする場合は、選択してください" persistent-hint name="mention"
                  v-model="formReportComment.mention_id" :items="mentions" item-value="id" item-text="name" ></v-select>
              </v-list-item>
