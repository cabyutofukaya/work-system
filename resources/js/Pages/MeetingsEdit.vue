...
export default {
  ...
  data() {
    return {
      // Inertia Formヘルパ
      form: this.$inertia.form(`MeetingsEdit${this.meeting.id}`, {
        ...this.meeting
      }),
      loading: {}
    };
  ...
  mounted() {
    ...
    this.form.started_at = this.form.started_at.replace(" ", "T");
  },

  methods: {
    // 会議記録更新実行
    update: function () {
      this.form.put(this.$route('meetings.update', {meeting: this.meeting.id}), {
        onStart: () => this.$set(this.loading, "update", true),
        onSuccess: () => this.$toasted.show('会議記録の変更を保存しました'),
