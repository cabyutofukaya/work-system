            ...
            <v-list>
              <v-list-item v-if="initSearch">
                <v-row>
                  <v-col
                      cols="12" sm="4"
                      class="text-center"
                      :class="{'pb-0' : Boolean($vuetify.breakpoint.xs)}"
                  >
                    <Button
                        block
                        :to="$route('client-types.clients.map', {client_type: this.client_type.id})"
                    >
                      <v-icon left>
                        mdi-map-outline
                      </v-icon>
              ...
              <v-list-item class="mt-4">
                <v-text-field
                    ...
                    name="word"
                    label="ワード検索"
                    hint="会社名(ふりがな)・電話番号から指定したワードを含む項目に絞り込みます" persistent-hint
                    maxlength="200"
                    v-model="form['word']"
              ...
              <v-list-item>
                <v-text-field
                    ...
                    name="address"
                    label="所在地検索"
                    hint="本部または営業所の所在地に指定したワードを含む項目に絞り込みます" persistent-hint
                    maxlength="200"
              ...
              <v-list-item v-if="client_type.id === 'taxibus'">
                <v-text-field
                    ...
                    label="営業エリア検索"
                    hint="営業エリアに指定したワードを含む項目に絞り込みます" persistent-hint
                    maxlength="200"
                    v-model="form['business_district']"
                ></v-text-field>
              </v-list-item>
              ...
              <v-list-item v-if="client_type['categories']">
                <v-select
                    ...
                    name="category"
                    label="カテゴリー"
                    hint="指定したカテゴリーの項目に絞り込みます" persistent-hint
                    v-model="form.category"
                    :items="Object.entries(client_type['categories']).map(([value, text]) => ({value, text}))"
                ></v-select>
              </v-list-item>
              ...
              <v-list-item>
                <v-select
                    ...
                    name="genre_id"
                    label="ジャンル"
                    hint="指定したジャンルの項目に絞り込みます" persistent-hint
                    maxlength="200"
              ...
              <v-list-item v-if="client_type.id === 'taxibus'">
                <v-text-field
                    ...
                    name="vehicle"
                    label="保有車両検索"
                    hint="保有車両名・説明文からワードを含む項目に絞り込みます" persistent-hint
                    maxlength="200"
