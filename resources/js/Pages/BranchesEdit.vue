...
export default {
  ...
  methods: {
    ...
    // 営業所情報作成
    update: function () {
      this.form.put(this.$route('branches.update', {branch: this.branch.id}), {
        onStart: () => this.$set(this.loading, "update", true),
        onSuccess: () => {
          this.$toasted.show('営業所情報を更新しました');
    ...
    destroy: function () {
      this.$confirm('この営業所を削除してよろしいですか？<br>削除した項目を元に戻すことはできません').then(isAccept => {
        if (isAccept) {
          this.form.delete(this.$route('branches.destroy', {branch: this.branch.id}), {
            onStart: () => this.$set(this.loading, "delete", true),
            onSuccess: () => {
              this.$toasted.show('営業所情報を削除しました');
