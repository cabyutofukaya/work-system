<template>
  <Layout>

    <v-container>
      <h3 class="mb-4">社内連絡事項一覧</h3>

      <v-card flat tile>
        <v-card-text>
          <v-row>
            <v-col class="text-right">
              <v-btn :small="$vuetify.breakpoint.xs" @click.native="showDialog = true" color="primary">
                <v-icon left>
                  mdi-plus
                </v-icon>
                社内連絡事項を作成する
              </v-btn>
            </v-col>
          </v-row>
        </v-card-text>

        <v-divider class="mx-4"></v-divider>

        <v-list>
          <div v-for="notice in notices['data']" :key="notice.id">
            <v-list-item dusk="noticeShow">
              <v-list-item-content>
                <v-row>
                  <v-col sm="2">
                    <v-btn :href="$route('notices.show', { id: notice.id })" target="_blank" small
                      class="mr-2">詳細</v-btn>
                  </v-col>
                  <v-col sm="2">
                    {{ notice.created_at }}
                  </v-col>
                  <v-col sm="3">
                    {{ notice.title }}
                  </v-col>
                  <v-col sm="2">
                    {{ notice.user.name }}
                  </v-col>
                </v-row>
              </v-list-item-content>
            </v-list-item>

            <v-divider class="mx-4"></v-divider>
          </div>
        </v-list>

        <v-card-text v-if="!notices.data.length">
          社内連絡事項はありません
        </v-card-text>

        <v-card-text>
          <v-pagination v-model="page" :length="length" @input="changePage"></v-pagination>
        </v-card-text>

      </v-card>

      <!-- 社内連絡事項追加ダイアログ -->
      <v-dialog v-model="showDialog" :max-width="$vuetify.breakpoint.smAndUp ? '600px' : 'unset'"
        @click:outside="form.clearErrors()">
        <v-card flat tile>
          <v-card-title>
            社内連絡事項の追加
          </v-card-title>

          <v-card-text>
            <v-text-field dense filled prepend-inner-icon="mdi-pencil" label="タイトル" name="title" v-model="form.title"
              maxlength="200" :error="Boolean(form.errors.title)" :error-messages="form.errors.title"></v-text-field>

            <v-textarea dense filled prepend-inner-icon="mdi-pencil" label="本文" name="description"
              v-model="form.description" :error="Boolean(form.errors.description)"
              :error-messages="form.errors.description"></v-textarea>

          </v-card-text>

          <v-card-text class="text-center">
            <v-btn color="primary" :small="$vuetify.breakpoint.xs" @click.native="create" :loading="loading['create']">
              <v-icon left>
                mdi-content-save-edit-outline
              </v-icon>
              この内容で追加する
            </v-btn>
          </v-card-text>

          <v-divider></v-divider>

          <v-card-text class="text-right">
            <v-btn class="mt-4" :small="$vuetify.breakpoint.xs" @click.native="form.clearErrors(); showDialog = false">
              <v-icon>
                mdi-close
              </v-icon>
              閉じる
            </v-btn>
          </v-card-text>
        </v-card>
      </v-dialog>
    </v-container>
  </Layout>
</template>

<script>
import Layout from "./Layout";
import { Link } from "@inertiajs/inertia-vue";

export default {
  components: { Layout, Link },

  props: ['notices'],

  data() {
    return {
      page: Number(this.notices['current_page']),
      length: Number(this.notices['last_page']),


      showDialog: false,
      form: this.$inertia.form({
        title: null,
        description: null,
      }),

      loading: {},
    }
  },

  methods: {
    // 社内連絡事項情報作成
    create: function () {
      this.form.post(this.$route('notices.store'), {
        preserveState: (page) => Object.keys(page.props.errors).length,
        onStart: () => this.$set(this.loading, "create", true),
        onSuccess: () => this.$toasted.show('社内連絡事項の作成を完了しました'),
        onFinish: () => this.$set(this.loading, "create", false),
      })
    },

    changePage() {
      this.$inertia.get(this.$route('notices.index'), {
        page: this.page,
      });
    },
  }
}
</script>
