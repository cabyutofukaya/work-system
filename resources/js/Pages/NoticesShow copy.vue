          ...
          <v-row>
            <v-col cols="3" v-for="(image, i_index) in images_list" :key="i_index">
              <v-card class="mx-auto" max-width="400">
                ...
                <div class="text-right" v-if="$page.props.auth.user.id == notice.user_id">
                  <v-icon color="error" class="text-right" @click.native="deleteFile(i_index)">
                    mdi-close-circle-outline</v-icon>
                </div>

                <v-img :src="`/storage/notice/${image}`" max-height="250" min-height="150" cover
        ...
        <v-container fluid>
          <v-row>
            <v-col cols="12" v-for="(image, i_index) in images_list" :key="i_index">
              <v-card>
                <v-img :src="`/storage/notice/${image}`" max-height="250" min-height="150"></v-img>
              </v-card>
            </v-col>
          </v-row>
        </v-container>
          ...
          <v-row>
            <v-col cols="12" v-for="(file, f_index) in files_list" :key="f_index" class="mb-3">
              ...
              <v-row>
                ...
                <a :href="`/storage/notice/${file.name}`" target="_blank">
                  {{ file.tmp_name }}
                </a>

                <div v-if="$page.props.auth.user.id == notice.user_id" class="ml-8">
