...
export default {
  ...
  props: ['client_types', 'client_type_taxibus_categories', 'genres', 'clients_total_count', 'clients_count', 'clients', 'users', 'sales_todo'],

  data() {
    return {
      // フォームヘルパ
      form: this.$inertia.form(`SalesTodosEdit${this.sales_todo.id}`, {
        ...this.sales_todo,
        participants: this.sales_todo["sales_todo_participants"].map((participant) => participant["user_id"]),
      }),
  ...
  methods: {
    ...
    getClients: function (client_id = null) {
      ...
      this.$inertia.post(this.$route('sales-todos.edit', {sales_todo: this.sales_todo.id}), param, {
        preserveState: true,
        preserveScroll: true,
        replace: true,
    ...
    // ToDo情報更新
    update: function () {
      this.form.put(this.$route('sales-todos.update', {sales_todo: this.sales_todo.id}), {
