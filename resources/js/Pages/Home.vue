<template>
  <Layout>
    ...
    <!-- 新着のお知らせ -->
    <v-card flat tile class="mt-6" dusk="notices">
      <v-card-title>
      ...
      <v-list>
        <div v-for="notice in notices" :key="notice.id">
          <Link :href="$route('notices.show', { id: notice.id })" exact as="v-list-item" dusk="noticeShow">
          <v-list-item-content>
      ...
      <v-card-text class="text-right" v-if="notices.length">
        <Button class="mb-2 mr-2" :small="$vuetify.breakpoint.xs" :to="$route('notices.index')" dusk="noticeIndex">
          <v-icon>
    ...
    <!-- 新着の議事録 -->
    <v-card flat tile class="mt-6" dusk="meeting" v-if="meetings.length">
      <v-card-title>
      ...
      <div class="mt-4 text-right" v-if="meetings.length">
        <Button class="mb-2 mr-2" :small="$vuetify.breakpoint.xs" :to="$route('meetings.index')" dusk="salesTodoIndex">
          <v-icon>
            mdi-format-list-bulleted
          </v-icon>
          すべての議事録を見る
    ...
    <!-- 直近の営業ToDoリスト  -->
    <v-card flat tile class="mt-6" dusk="sales_todos">
      <v-card-title>
              ...
              <v-row :class="{ 'grey': sales_todo['is_completed'], 'lighten-4': sales_todo['is_completed'] }">
                <v-col cols="12" sm="3" class="grey lighten-3 px-6">
                  ...
                  <div class="d-flex align-center">
                    <div>
                      <Link :href="$route('clients.show', { client: sales_todo['client_id'] })"
                        dusk="clientShowSalesTodo">
                      <v-list-item-avatar tile>
                        <v-img :src="sales_todo['client']['icon_image_url']" alt=""></v-img>
                      </v-list-item-avatar>
                      </Link>
                    </div>
                ...
                <template v-if="!$vuetify.breakpoint.xs">
                  <v-col cols="1">
                    <div v-for="(sales_todo_participant, index) in sales_todo['sales_todo_participants']" :key="index"
                      class="d-inline-block mr-2" :class="{ 'mt-1': index !== 0 }" style="white-space:nowrap">
                      <Link :href="$route('users.show', { user: sales_todo_participant['user_id'] })"
                        dusk="userShowSalesTodoParticipant">
                ...
                <template v-else>
                  <v-col cols="12" v-if="sales_todo['sales_todo_participants'].length">
                    社内担当者：<span v-for="(sales_todo_participant, index) in sales_todo['sales_todo_participants']"
                ...
                <v-col cols="12" sm="3" class="text-right">
                  <Button v-if="!sales_todo['is_completed']" color="primary" dark class="ml-2 mb-1"
                    :small="$vuetify.breakpoint.xs" @click.native.prevent="closeSalesTodo(sales_todo.id)"
                    :loading="loading['sales-todo-complete-' + sales_todo.id]" dusk="salesTodoComplete">
                    <v-icon left>
                  ...
                  <Button v-if="sales_todo['is_completed']" color="primary" outlined class="ml-2 mb-1"
                    :small="$vuetify.breakpoint.xs" @click.native.prevent="openSalesTodo(sales_todo.id)"
                    :loading="loading['sales-todo-complete-' + sales_todo.id]" dusk="salesTodoComplete">
                    <v-icon left>
                      mdi-checkbox-marked-outline
                    </v-icon>
                  ...
                  <Button class="ml-2 mb-1" :color="sales_todo['is_completed'] ? 'grey' : undefined"
                    :outlined="sales_todo['is_completed']" :small="$vuetify.breakpoint.xs"
                    :to="$route('sales-todos.edit', { sales_todo: sales_todo.id })" dusk="salesTodoEdit">
                    <v-icon left>
                      mdi-pencil
                    </v-icon>
                    編集
                  ...
                  <Button class="ml-2 mb-1" color="error" :outlined="sales_todo['is_completed']"
                    :small="$vuetify.breakpoint.xs" @click.native.prevent="deleteSalesTodo(sales_todo.id)"
                    :loading="loading['delete-sales-todo-' + sales_todo.id]" dusk="salesTodoDelete">
                    <v-icon left>
                      mdi-delete-outline
                    </v-icon>
                    削除
    ...
    <!-- 直近の社内ToDoリスト  -->
    <v-card flat tile class="mt-6" dusk="office_todos">
      <v-card-title>
        <v-icon dark left>mdi-calendar</v-icon>
        直近の社内ToDoリスト
      </v-card-title>
          ...
          <v-list-item>
            <v-list-item-content>
              <v-row :class="{ 'grey': office_todo['is_completed'], 'lighten-4': office_todo['is_completed'] }">
                ...
                <v-col cols="12" sm="3" class="text-right">
                  <Button v-if="!office_todo['is_completed']" color="primary" dark class="ml-2 mb-1"
                    :small="$vuetify.breakpoint.xs" @click.native.prevent="closeOfficeTodo(office_todo.id)"
                    :loading="loading['office-todo-complete-' + office_todo.id]" dusk="officeTodoComplete">
                    <v-icon left>
                      mdi-checkbox-marked-outline
                    </v-icon>
                    完了
                  ...
                  <Button v-if="office_todo['is_completed']" color="primary" outlined class="ml-2 mb-1"
                    :small="$vuetify.breakpoint.xs" @click.native.prevent="openOfficeTodo(office_todo.id)"
                    :loading="loading['office-todo-complete-' + office_todo.id]" dusk="officeTodoComplete">
                    <v-icon left>
                      mdi-checkbox-marked-outline
                    </v-icon>
                  ...
                  <Button class="ml-2 mb-1" :color="office_todo['is_completed'] ? 'grey' : undefined"
                    :outlined="office_todo['is_completed']" :small="$vuetify.breakpoint.xs"
                    :to="$route('office-todos.edit', { office_todo: office_todo.id })" dusk="officeTodoEdit">
                    <v-icon left>
                      mdi-pencil
                    </v-icon>
                    編集
                  ...
                  <Button class="ml-2 mb-1" color="error" :outlined="office_todo['is_completed']"
                    :small="$vuetify.breakpoint.xs" @click.native.prevent="deleteOfficeTodo(office_todo.id)"
                    :loading="loading['delete-office-todo-' + office_todo.id]" dusk="officeTodoDelete">
                    <v-icon left>
                      mdi-delete-outline
                    </v-icon>
                    削除
      ...
      <div class="mt-4 text-right" v-if="office_todos.length">
        <Button class="mb-2 mr-2" :small="$vuetify.breakpoint.xs" :to="$route('office-todos.index')"
    ...
    <!-- 最新の日報 -->
    <v-card flat tile class="mt-6" dusk="reports">
      <v-card-title>
      ...
      <v-card-text>
        <v-row>
          ...
          <v-col class="text-right">
            <Button :small="$vuetify.breakpoint.xs" :to="$route('reports.create')">
              <v-icon left>
                mdi-plus
              </v-icon>
              日報を作成する
            ...
            <v-row>
              ...
              <template v-else>
                <v-col cols="12">
                  ...
                  <div><span v-if="report['report_contents_sales_exists']">営業日報</span><span
                      v-if="report['report_contents_sales_exists'] && report['report_contents_work_exists']">・</span><span
                      v-if="report['report_contents_work_exists']">業務日報</span>
                  </div>
                </v-col>
      ...
      <div class="mt-4 text-right" v-if="reports_latest_comment.length">
        <Button class="mb-2 mr-2" :small="$vuetify.breakpoint.xs" :to="$route('reports.index')">
          <v-icon>
            mdi-format-list-bulleted
          </v-icon>
          すべての日報を見る
    ...
    <!-- コメントがついた日報 -->
    <v-card flat tile class="mt-6" dusk="reports_latest_comment">
      <v-card-title>
        <v-icon dark left>mdi-file-document</v-icon>
        コメントがついた日報
      </v-card-title>
