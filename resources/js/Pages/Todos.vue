<template>
  <Layout>
            ...
            <v-list-item-content>
              <v-row :class="{ 'done-row': todo.is_done }">
                ...
                <v-col cols="12" sm="4" class="text-right">
                  <v-btn icon :color="todo.is_done ? 'grey' : 'success'" @click="toggleDone(todo)">
                    <v-icon>
                      {{ todo.is_done ? 'mdi-undo' : 'mdi-check' }}
                    </v-icon>
                  </v-btn>
    ...
    <v-dialog v-model="editDialog" max-width="500px">
      <v-card>
        <v-card-title class="text-h6 font-weight-bold">TODOを編集</v-card-title>

        <v-divider></v-divider>

        <v-card-text class="pt-4 pb-0">
          <v-text-field v-model="formEdit.date" label="日付" type="date" dense outlined class="mb-4" />
          <v-text-field v-model="formEdit.title" label="TODO" dense outlined />
        </v-card-text>
    ...
    <v-dialog v-model="createDialog" max-width="500px">
      <v-card>
        <v-card-title class="text-h6 font-weight-bold">TODOを追加</v-card-title>

        <v-divider></v-divider>

        <v-card-text class="pt-4 pb-0">
          <v-text-field v-model="formCreate.date" label="日付" type="date" dense outlined class="mb-4" />
          <v-text-field v-model="formCreate.title" label="TODO" dense outlined />
        </v-card-text>
...
export default {
  ...
  methods: {
    create() {
      if (!this.formCreate.date || !this.formCreate.title) {
        this.$toasted.error('日付とTODO内容は必須です');
        return;
      }
    ...
    submitEdit() {
      if (!this.formEdit.date || !this.formEdit.title) {
        this.$toasted.error('日付とTODO内容は必須です');
        return;
      }
    ...
    toggleDone(todo) {
      ...
      Inertia.patch(this.$route('todos.toggleDone', { todo: todo.id }), {}, {
        onSuccess: () => {
          this.$toasted.show(
            nextState ? 'TODOを完了にしました' : 'TODOを未完了に戻しました'
          );
        },
      });
    }
